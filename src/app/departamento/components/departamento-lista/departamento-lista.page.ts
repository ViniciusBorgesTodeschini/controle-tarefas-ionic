...
export class DepartamentoPage implements OnInit {
  ...
  listar() {
    this.departamentoService.getAtendimentosMeios().subscribe(
      (dados) => {
  ...
  confirmarExclusao(departamento: DepartamentoInterface) {
    this.alertController
      .create({
        ...
        message: `Deseja excluir o assunto ${departamento.nome}?`,
        buttons: [
          {
            text: 'Sim',
      ...
      this.departamentoService.excluir(departamento.id).subscribe(
        ...
        (erro) => {
          ...
          this.toastController
            .create({
              message: `Não foi possível excluir o assunto ${departamento.nome}`,
              duration: 5000,
              keyboardClose: true,
              color: 'danger',
            })
